...
local function updateSpider()
    ...
    if spiderEnabled and LocalPlayer.Character then
        local humanoid = LocalPlayer.Character:FindFirstChild("Humanoid")
        local rootPart = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        
        if humanoid and rootPart then
            spiderConnection = RunService.Heartbeat:Connect(function()
                if spiderEnabled and humanoid and rootPart then
                    -- Проверяем, находится ли персонаж рядом со стеной
                    ...
                    if raycastResult then
                        -- Если рядом стена, отключаем гравитацию и позволяем взбираться
                        humanoid.PlatformStand = true
                        rootPart.Velocity = Vector3.new(rootPart.Velocity.X, 0, rootPart.Velocity.Z)
                        
                        -- Движение вверх/вниз при нажатии клавиш
                        ...
                        -- Если нет стены, возвращаем нормальное состояние
...
createButton(scroll, "Spider [OFF]", function()
	spiderEnabled = not spiderEnabled
    updateSpider()
    
    -- Обновляем текст кнопки
    local button = scroll:FindFirstChildOfClass("TextButton")
    if button then
        button.Text = spiderEnabled and "Spider [ON]" or "Spider [OFF]"
    end
end)
